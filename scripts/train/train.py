...
from qvc.data_loader import (
    DistributedBucketSampler,
    TextAudioSpeakerCollate,
    TextAudioSpeakerLoader,
)
from qvc.losses import (
    discriminator_loss,
    feature_loss,
    generator_loss,
    kl_loss,
...
from qvc.mel_processing import mel_spectrogram_torch, spec_to_mel_torch
from qvc.models import MultiPeriodDiscriminator, SynthesizerTrn
from qvc.pqmf import PQMF
...
def run(rank, n_gpus, hps):
    ...
    try:
        ...
        print(global_step, len(train_loader))
    except:
        epoch_str = 1
        global_step = 0
    ...
    for batch_idx, (c, spec, y) in enumerate(train_loader):
        ...
        with torch.amp.autocast("cuda", enabled=hps.train.fp16_run):
            ...
            if np.random.random() < 0.2:
                scale = np.random.uniform(0.1, 0.01)
                c = c + torch.randn_like(c) * scale
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                values = [x.item() for x in losses] + [global_step, lr]
                info_str = ", ".join(
